...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event) {
  ...
  switch (event->event_id) {
    ...
    case MQTT_EVENT_DATA:
      ...
      // Evento trigado quando recebe uma mensagem da Cloud

      trataMensagemC2D(String(incoming_data));
...
static void generateTelemetryPayload() {
  ...
  jsonObject["LEDStatus"] = String(led_status);

  // Mais informações para envio podem ser adicionadas aqui :)

  telemetry_payload = JSON.stringify(jsonObject);
}
...
void trataMensagemC2D(String mensagem) {
  JSONVar objeto = JSON.parse(mensagem);
  if (objeto.hasOwnProperty("LedLigado")) {
    led_status = (bool)objeto["LedLigado"];
